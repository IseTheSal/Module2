...
class CertificateServiceImplTest {
    ...
    public void setUp() {
        ...
        service = new GiftCertificateServiceImpl(giftRepository, tagRepository, messageSource);
    }
    ...
    void findAll() {
        ...
        Mockito.when(giftRepository.findAll(PageRequest.of(0, 100))).thenReturn(giftCertificates);
    ...
    void findByParametersTagNotException() {
        Mockito.when(tagRepository.findByName(MockData.TAG_ONE.getName())).thenThrow(new TagNotFoundException());
    ...
    @Test
    void findById() {
        Mockito.when(giftRepository.findById(1L)).thenReturn(Optional.ofNullable(MockData.GIFT_CERTIFICATE));
    ...
    @Test
    void create() {
        Mockito.when(giftRepository.save(MockData.GIFT_CERTIFICATE)).thenReturn(MockData.GIFT_CERTIFICATE);
    ...
    @Test
    void update() {
        Mockito.when(giftRepository.findById(1L)).thenReturn(Optional.of(MockData.GIFT_CERTIFICATE));
        Mockito.when(giftRepository.save(ArgumentMatchers.any())).thenReturn(MockData.UPDATED_GIFT_CERTIFICATE);
    ...
    void deleteThrowException() {
        Mockito.when(giftRepository.findById(9L)).thenReturn(Optional.empty());
